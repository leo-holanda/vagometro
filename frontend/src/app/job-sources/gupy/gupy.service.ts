...
export class GupyService {
  ...
  getJobsFromMongo(collectionName: string, quarterData: QuarterData): Observable<Job[]> {
    ...
    return this.mongoService.getJobs<GupyJob[]>(collectionName).pipe(
      ...
      map((event) => (event as HttpResponse<GupyJob[]>).body),
  ...
  private mapJobsWithWorker(jobs: GupyJob[] | null, quarterData: QuarterData): Observable<Job[]> {
    return defer(() => {
  ...
  private updateDownloadProgress(event: HttpEvent<GupyJob[]>, quarterData: QuarterData): void {
